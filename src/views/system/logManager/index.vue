<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table  :key='tableKey' :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit highlight-current-row
      ...
      <el-table-column width="300px" align="center" :label="tableCol.username">
        <template slot-scope="scope">
          <span>{{scope.row.username }}</span>
        </template>
      </el-table-column>
      <el-table-column min-width="50px" :label="tableCol.detail">
        <template slot-scope="scope">
...
<script>
import { fetchList, createLog, updateLog } from '@/api/log'
...
export default {
  ...
  data() {
    return {
      ...
      tableData: null,
      tableHeader: null,
      tHeader: ['id', 'username', 'detail', 'ip', 'time']
    }
  },
  filters: {
    statusFilter(status) {
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const filterVal = ['id', 'username', 'detail', 'ip', 'time']
